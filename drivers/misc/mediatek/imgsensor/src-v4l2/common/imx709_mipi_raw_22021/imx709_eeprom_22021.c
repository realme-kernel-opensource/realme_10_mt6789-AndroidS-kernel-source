...
static bool selective_read_eeprom(struct subdrv_ctx *ctx, kal_uint16 addr,
                  BYTE *data)
{
    ...
    if (adaptor_i2c_rd_u8(ctx->i2c_client, IMX709_EEPROM_SLAVE_ADDRESS >> 1,
                  addr, data) < 0) {
        return false;
    }
    return true;
...
static bool read_imx709_eeprom(struct subdrv_ctx *ctx, kal_uint16 addr,
                   BYTE *data, int size)
{
    int i = 0;
    int offset = addr;
...
unsigned int read_imx709_LRC_22021(struct subdrv_ctx *ctx, kal_uint16 *data)
{
    ...
    if ((readed_size == 0) &&
        read_imx709_eeprom(ctx, pinfo->LRC_addr, imx709_LRC_data,
                   pinfo->LRC_size)) {
    ...
    for (idx = 0; idx < LRC_SIZE; idx++) {
        if (idx < LRC_SIZE / 2) {
            //LRC_Left
            pr_debug("In %s: LRC_Left value[0x%x]:0x%x", __func__,
                 data[2 * idx], data[2 * idx + 1]);
        } else {
            //LRC_RIGHT
            pr_debug("In %s: LRC_Right value[0x%x]:0x%x", __func__,
                 data[2 * idx], data[2 * idx + 1]);
        }
    }
    read_imx709_eeprom(ctx, 0x162E, &lrc_flag, 1);
    pr_info("LRC flag0x%x[1:valid, other:Invalid]", lrc_flag);
    return readed_size;
}

unsigned int read_imx709_QSC_22021(struct subdrv_ctx *ctx, kal_uint8 *data)
{
    kal_uint16 idx = 0;
    kal_uint8 qsc_ver = 0;

    read_imx709_eeprom(ctx, OTP_QSC_OFFSET, data, QSC_SIZE);

    for (idx = 0; idx < QSC_SIZE; idx++) {
        pr_debug("alex qsc data imx709_QSC_setting[0x%x] = 0x%x", idx, data[idx]);
    }

    read_imx709_eeprom(ctx, 0x2A32, &qsc_ver, 1);
    pr_info("QSC Version: 0x%x", qsc_ver);
    return 0;
}

struct eeprom_map_info imx709_eeprom_info_22021[] = {
...
unsigned int read_imx709_eeprom_info_22021(struct subdrv_ctx *ctx, kal_uint16 meta_id,
                     BYTE *data, int size)
{
    kal_uint16 addr;
    int readsize;

    if (meta_id != imx709_eeprom_info_22021[meta_id].meta)
        return -1;

    if (size != imx709_eeprom_info_22021[meta_id].size)
        return -1;

    addr = imx709_eeprom_info_22021[meta_id].start;
    readsize = imx709_eeprom_info_22021[meta_id].size;

    read_imx709_eeprom(ctx, addr, data, readsize);
